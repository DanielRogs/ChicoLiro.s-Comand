...
const int velocidadeBase = 90;
const int velocidadeMaxima = 100;
...
const unsigned long interval = 4000; // Intervalo de 1 segundo
...
const int limiteIdentificador = 40000;
int tempoPerdido = 0;
int tempoParado = 0;
int tempoNoPreto = 0;
...
// Declaração de uma lista para armazenar valores de sensores
std::list<float> RPM_Esquedo;
std::list<float> RPM_Direito;
std::list<double> Voltage;
// std::list<double> Corrente;

// Função Exemplo para ler os sensores e calcular as RPMs
void calculateSpeed() {
  RPM_Esquedo.push_back(((counter_A / diskslots) * 60.00)/2);
  RPM_Direito.push_back(((counter_B / diskslots) * 60.00)/2);
...
void calculateTension() {
  ...
  Voltage.push_back(((adc_pin35 * 5)/4096) * 0.8333); 
}
...
void lerSensores() {
  ...
  if (sensorMeio > limitePreto && sensorEsq <= limitePreto && sensorDir <= limitePreto) {
    ...
    // Fim do percurso - Mas ver se é curva
      digitalWrite(IN1, LOW);
      digitalWrite(IN2, HIGH);
      analogWrite(EN_A, velocidadeBase - ajusteVelocidade);
    ...
    if (tempoNoPreto >= 10000) {
    // Fim do percurso - Carro realmente chegou ao fim
      digitalWrite(IN1, LOW);
      digitalWrite(IN2, LOW);
      analogWrite(EN_A, 0);
...
// // Função para converter lista em string JSON
// String listaParaJSONFloat(std::list<float>& lista) {
//   String json = "[";
...
// // Função para converter lista em string JSON
// String listaParaJSONDouble(std::list<double>& lista) {
//   String json = "[";
//   for (auto it = lista.begin(); it != lista.end(); ++it) {
//     if (it != lista.begin()) {
//       json += ",";
...
//       // Converta a lista para uma string JSON
//       String RPM_DireitoJSON = listaParaJSONFloat(RPM_Direito);
//       String RPM_EsquedoJSON = listaParaJSONFloat(RPM_Esquedo);
//       String VoltageJSON = listaParaJSONDouble(Voltage);

//       String jsonData = "{\"rpmMotorDir\": " + RPM_DireitoJSON +
//                   ", \"rpmMotorEsq\": " + RPM_EsquedoJSON +
//                   ", \"tensao\": " + VoltageJSON + "}";
